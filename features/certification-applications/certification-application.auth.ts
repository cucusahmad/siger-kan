...
export function certificationError(error: unknown) {
  ...
  const errors: Readonly<Record<string, { status: number; message: string }>> = {
    ...
    BUSINESS_REQUIRED: { status: 403, message: "Akun belum terhubung ke profil usaha." }, NOT_FOUND: { status: 404, message: "Permohonan sertifikasi tidak ditemukan." },
    INVALID_STATUS: { status: 409, message: "Permohonan tidak dapat diproses pada status saat ini." }, DOCUMENT_REQUIRED: { status: 422, message: "Unggah minimal satu dokumen persyaratan." },
