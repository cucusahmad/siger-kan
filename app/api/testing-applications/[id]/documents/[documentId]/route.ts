...
import { requireSampleReceptionOfficer, requireTestingReportViewer } from "@/features/testing-applications/testing-application.auth";
...
export async function GET(_request: Request, context: Context) { try { const { id, documentId } = await context.params; if (!validId(id) || !validId(documentId)) throw new Error("NOT_FOUND"); let includeAllStatuses = false; try { await requireTestingReportViewer(); includeAllStatuses = true; } catch { await requireSampleReceptionOfficer(); } return await serveApplicationDocumentForReception(id, documentId, includeAllStatuses); } catch (error: unknown) { return apiError(error); } }
