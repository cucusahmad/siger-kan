...
import { Building2, CheckCircle2, Clock3, Eye, Mail, MapPin, Phone, Search, UsersRound } from "lucide-react";
...
export function AdminDashboard({ adminName, data }: AdminDashboardProps) {
  ...
  const availableTypes = useMemo(() => [...new Set(data.businesses.map((business) => business.businessType))].sort(), [data.businesses]);
  const availableRegions = useMemo(() => [...new Set(data.businesses.map((business) => business.region))].sort(), [data.businesses]);
  const filteredBusinesses = useMemo(() => {
    const keyword = query.trim().toLocaleLowerCase("id-ID");
    return data.businesses.filter((business) => {
      const matchesQuery = !keyword || [business.name, business.code, business.ownerName, business.ownerEmail, business.phone, business.location]
        .some((value) => value.toLocaleLowerCase("id-ID").includes(keyword));
      return matchesQuery
        && (status === "ALL" || business.status === status)
        && (businessType === "ALL" || business.businessType === businessType)
        && (region === "ALL" || business.region === region);
    });
  }, [businessType, data.businesses, query, region, status]);
  ...
  return (
    <div>
      <header className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
        <div><p className="text-xs font-bold uppercase tracking-[.14em] text-ocean">Data Pelaku Usaha</p><h1 className="mt-2 text-2xl font-bold text-navy sm:text-3xl">Selamat datang, {adminName}</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-muted">Pantau data pengguna dan kelengkapan informasi pelaku usaha perikanan dalam satu ruang kerja.</p></div>
      ...
      <section aria-labelledby="business-list-title" className="mt-7 overflow-hidden rounded-3xl border border-slate-200/80 bg-white shadow-[0_12px_40px_rgba(7,59,76,.05)]">
        <div className="border-b border-slate-100 p-5 sm:p-6"><h2 id="business-list-title" className="text-lg font-bold text-navy">Data Pengguna Pelaku Usaha</h2><p className="mt-1 text-sm text-muted">Cari pelaku usaha dan buka detail profil, legalitas, serta komoditasnya.</p><div className="mt-5 grid gap-3 md:grid-cols-2 xl:grid-cols-[minmax(260px,1fr)_190px_190px_190px]"><label className="relative"><span className="sr-only">Cari pelaku usaha</span><Search className="absolute left-3.5 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400" /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Cari nama, email, telepon, atau alamat..." className="min-h-11 w-full rounded-xl border border-slate-200 bg-slate-50 pl-10 pr-4 text-sm text-navy outline-none transition focus:border-ocean focus:bg-white focus:ring-2 focus:ring-ocean/10" /></label><FilterSelect label="Filter status usaha" value={status} onChange={setStatus}><option value="ALL">Semua status</option>{Object.entries(statusLabels).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</FilterSelect><FilterSelect label="Filter jenis usaha" value={businessType} onChange={setBusinessType}><option value="ALL">Semua jenis usaha</option>{availableTypes.map((value) => <option key={value} value={value}>{typeLabels[value] ?? value}</option>)}</FilterSelect><FilterSelect label="Filter wilayah" value={region} onChange={setRegion}><option value="ALL">Semua wilayah</option>{availableRegions.map((value) => <option key={value} value={value}>{value}</option>)}</FilterSelect></div></div>
        <div className="overflow-x-auto"><table className="w-full min-w-[1120px] text-left text-sm"><thead className="bg-slate-50 text-[11px] uppercase tracking-wider text-muted"><tr><th className="px-6 py-4">Pelaku usaha</th><th className="px-4 py-4">Pemilik</th><th className="px-4 py-4">Kontak</th><th className="px-4 py-4">Jenis & alamat</th><th className="px-4 py-4">Kelengkapan</th><th className="px-4 py-4">Status</th><th className="px-6 py-4 text-right">Aksi</th></tr></thead><tbody className="divide-y divide-slate-100">{filteredBusinesses.map((business) => <tr key={business.id} className="transition hover:bg-slate-50/70"><td className="px-6 py-4"><p className="font-bold text-navy">{business.name}</p><p className="mt-1 text-xs text-muted">{business.code} · Terdaftar {formatDate(business.createdAt)}</p></td><td className="px-4 py-4"><p className="font-semibold text-ink">{business.ownerName}</p></td><td className="px-4 py-4"><p className="flex items-center gap-1.5 text-xs text-muted"><Mail className="h-3.5 w-3.5 shrink-0" />{business.ownerEmail}</p><p className="mt-2 flex items-center gap-1.5 text-xs text-muted"><Phone className="h-3.5 w-3.5 shrink-0" />{business.phone}</p></td><td className="max-w-xs px-4 py-4"><p className="font-semibold text-ink">{typeLabels[business.businessType] ?? business.businessType}</p><p className="mt-1 flex items-start gap-1.5 text-xs leading-5 text-muted"><MapPin className="mt-0.5 h-3.5 w-3.5 shrink-0" />{business.location}</p></td><td className="px-4 py-4"><p className="text-xs font-semibold text-ink">{business.documentCount} dokumen</p><p className="mt-1 text-xs text-muted">{business.commodityCount} komoditas</p></td><td className="px-4 py-4"><span className={`inline-flex rounded-full px-3 py-1.5 text-[11px] font-bold ${business.status === "ACTIVE" ? "bg-[#EAF7F0] text-[#247D55]" : business.status === "PENDING_VERIFICATION" ? "bg-[#FFF7E2] text-[#8A6411]" : "bg-slate-100 text-slate-600"}`}>{statusLabels[business.status] ?? business.status}</span></td><td className="px-6 py-4 text-right"><Link href={`/dashboard/pelaku-usaha/${business.id}`} className="inline-flex min-h-10 items-center gap-2 rounded-xl border border-ocean/20 px-3.5 text-xs font-bold text-ocean transition hover:bg-seafoam focus-visible:outline-2 focus-visible:outline-ocean"><Eye className="h-4 w-4" />Lihat detail</Link></td></tr>)}</tbody></table></div>
...
function FilterSelect({ label, value, onChange, children }: FilterSelectProps) {
  return <label><span className="sr-only">{label}</span><select aria-label={label} value={value} onChange={(event) => onChange(event.target.value)} className="min-h-11 w-full rounded-xl border border-slate-200 bg-slate-50 px-4 text-sm font-semibold text-navy outline-none focus:border-ocean">{children}</select></label>;
}
